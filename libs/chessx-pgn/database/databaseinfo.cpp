...
DatabaseInfo::DatabaseInfo(QUndoGroup* undoGroup, const QString& fname): m_filter(0), m_index(InvalidGameId)
{
    ...
    else if(file.size() < 1024 * 1024 * 1)/* replace by loloof64 AppSettings->getValue("/General/EditLimit").toInt()) */
    {
...
bool DatabaseInfo::saveGame()
{
    ...
    QString eco;
    if(true /* replace by loloof64 AppSettings->getValue("/General/automaticECO").toBool() */)
    {
